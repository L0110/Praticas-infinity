...
function enviarFormulario(event) {
    ...
    try {
        ...
        if (!nome || !usuario || !senha || !email || !idade) {
...
function sucesso() {
    aviso.innerHTML=`<p style='color: white; background-color: #185000ff; border-radius: 5px; box-shadow: #3e3e3e 2px 2px 0px 0px; border: 2px solid white; width: fit-content;'>✅ Cadastro realizado com sucesso!</p>`;
}
function erro(mensagem) {
    aviso.innerHTML = `<p style='color: white; background-color: #9a0000ff; border-radius: 5px; box-shadow: #3e3e3e 2px 2px 0px 0px; border: 2px solid white; width: fit-content;'>❌ Erro: ${mensagem}</p>`;
}